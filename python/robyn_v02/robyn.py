...
class Robyn(object):
    ...
    def mmm(self,
        ...
        dt_decompSpendDist = xDecompAgg.loc[xDecompAgg.rn.isin(self.paid_media_vars), ["rn", "xDecompPerc", "xDecompMeanNon0Perc", "xDecompMeanNon0", "xDecompPercRF", "xDecompMeanNon0PercRF", "xDecompMeanNon0RF"]]
        dt_decompSpendDist = pd.merge(dt_decompSpendDist,dt_spendShare.loc[:,["rn", "spend_share", "mean_spend", "total_spend"]],how="left",on="rn")
        ...
        xDecompAgg = pd.merge(xDecompAgg,dt_decompSpendDist.loc[:,["rn", "spend_share_refresh", "effect_share_refresh"]],how="left",on="rn")
        xDecompAgg["spend_share_refresh"] = xDecompAgg["i.spend_share_refresh"] # not sure these variable are created with the same names in python (ie: with "i." in front)
        xDecompAgg["effect_share_refresh"] = xDecompAgg["i.effect_share_refresh"] # not sure these variable are created with the same names in python
        ...
        if not refresh :
            ...
            dt_decompRF = pd.merge(xDecompAgg["rn","decomp_perc"],self.xDecompAggPrev.loc[:,["rn", "decomp_perc_prev"]],how="left",on="rn")
            rssd_nonmedia = dt_decompRF.loc[~dt_decompRF['rn'].isin(self.paid_media_vars), "decomp_perc"] - dt_decompRF.loc[~dt_decompRF['rn'].isin(self.paid_media_vars), "decomp_perc_prev"]
            decomp_rssd_nonmedia = math.sqrt(rssd_nonmedia.pow(2).mean())
            rssd_media = dt_decompSpendDist.loc['effect_share_refresh'] - dt_decompSpendDist.loc['spend_share_refresh']
        ...
        if hyper_fixed==True :
            ...
            xDecompVec['intercept'] = xDecompAgg.loc[rn=="(intercept)", 'xDecompAgg'] #!! not sure about the argument 'xDecompAgg'
        ...
        if cnt == iterTotal : #!! probably should use self.cnt and self.iterTotal
            print("===")
            print(["Optimizer_name: "+ optimizer_name + ";  Total_iterations: " + cnt + ";   best_mape: "+ best_mape])
